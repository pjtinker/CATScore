                    ...
                    for model, selected in self.selected_models['sklearn'].items():
                        ...
                        if selected:
                            try:
                                ...
                                self._update_log(
                                    f'Evaluation Accuracy: {model_acc}', False)
                                self._update_log(
                                ...
                                self.save_model(
                                    model, pipeline, save_path, model_acc)
                            except (KeyboardInterrupt, SystemExit):
                                raise
        ...
        for args, values in param_dict.items():
            ...
            if current_type == 'feature_extraction':
                ...
                if include_feature_selection:
                    print("We want feature_selection!!!")
                    priority = 50
                else:
                    print("We're skipping feature_selection!!!")
                    continue
            else:
                priority = 100
    ...
    def save_model(self, model_name, pipeline, save_path, score):
        ...
        if model_name == 'TPOTClassifier':
            ...
            self.save_params_to_file(
                model_name, pipeline.get_params(), save_path, score)
