...
# Limit on the number of features. Using the top 20K features.
TOP_K = 20000
...
def train_fine_tuned_sequence_model(data,
    ...
    (test_numbers, test_texts, test_labels) = test_data
    ...
    # Vectorize texts.
    x_train, x_val, x_test, word_index = vectorize_data.sequence_vectorize(
    ...
    explore_data.plot_history(history)
    # Print results.
    ...
    print("Tuned SeqCNN Score", score)
    print("Tuned SeqCNN Accuracy", acc)
    
    predictions = model.predict_classes(x_test)
    # predictions = np.argmax(predictions, axis=1)
    preds = pd.DataFrame({'seq_cnn_predictions': predictions.flatten()}, index=test_numbers)
    kappa = explore_data.get_kappa(test_labels, predictions)
    print("Tuned SeqCNN Kappa", kappa)
    return score, acc, preds
    #preds = pd.DataFrame({'predictions': predictions })
    #preds.to_csv("tuned_sequence_model_predictions.csv", index=True)
    # Save model.
...
if __name__ == '__main__':
    ...
    parser.add_argument('--embedding_data_dir', type=str, default='D:/Research/TextClassification/glove6b',
                        help='embedding input data directory')
    FLAGS, unparsed = parser.parse_known_args()

    data = data_utils.load_data("D:/CSC/CSC6230MachineLearning/pathology_data_complete.csv")

    kappa, acc, predictions = train_fine_tuned_sequence_model(data, test_data, FLAGS.embedding_data_dir)